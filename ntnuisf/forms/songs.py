...
from ..models.songs import Song, Tag, File
# End: imports -----------------------------------------------------------------
...
class SongSearchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['tag'].choices += [(tag.id, tag.title) for tag in Tag.get_queryset(['song'])]
...
class SongForm(forms.ModelForm):
    tags = forms.ModelMultipleChoiceField(
        queryset=Tag.get_queryset(['song']), widget=FilteredSelectMultiple(verbose_name='tags', is_stacked=False), required=False
    )
    ...
    class Meta:
        model = Song
        exclude = []  # pylint: disable=modelform-uses-exclude,fixme # TODO: fix
